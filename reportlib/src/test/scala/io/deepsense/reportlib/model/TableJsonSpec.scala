...
class TableJsonSpec extends AnyWordSpec with Matchers with TableTestFactory with ReportJsonProtocol {
  ...
  "Table" should {
    "serialize" when {
      ...
      "rowsNames, columnNames and columTypes specified" in {
        val json = testTableWithLabels(Some(columnNames), columnTypes, Some(rowNames), values).toJson
    ...
    "deserialize" when {
      "filled table" in {
        ...
        json.convertTo[Table] shouldBe testTableWithLabels(columnNames, columnTypes, rowNames, values)
      }
      "empty table" in {
  ...
  private def jsonTable(
    ...
    Map[String, JsValue](
      ...
      "values" ->
        JsArray(values.map(row => JsArray(row.map(op => op.map(JsString(_)).getOrElse(JsNull)).toVector)).toVector)
    )
  )

  def toJsValue(values: Option[List[String]]): JsValue with Product with Serializable =
    values
      .map(values => JsArray(values.map(JsString(_)).toVector))
      .getOrElse(JsNull)
