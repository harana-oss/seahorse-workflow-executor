...
trait NodeStatusJsonProtocol extends DefaultJsonProtocol with FailureDescriptionJsonProtocol with NullOptions {
  ...
  implicit object NodeStatusFormat extends JsonFormat[NodeStatus] with DefaultJsonProtocol with NullOptions {
    ...
    override def write(state: NodeStatus): JsValue = {
      ...
      val view = state match {
        ...
        case Completed(started, ended, results) =>
          NodeStatusView(state.name, started = Some(started), ended = Some(ended), results = Some(results))
        case Failed(started, ended, error) =>
          NodeStatusView(state.name, started = Some(started), ended = Some(ended), error = Some(error))
