...
case class DatasourceIdForWriteParameter(override val name: String, override val description: Option[String])
    extends ParameterWithJsFormat[UUID] {

  override def replicate(name: String): DatasourceIdForWriteParameter = copy(name = name)
