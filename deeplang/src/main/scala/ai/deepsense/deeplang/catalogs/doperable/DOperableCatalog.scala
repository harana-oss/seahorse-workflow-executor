...
/** Allows to register and validate hierarchy of DClasses, DTraits and DOperations. Exposes tools for advance reflecting
...
class DOperableCatalog {
  ...
  /** Registers type - either trait or class - in catalog. All of this type's superclasses and supertraits that are
    * subtypes of DOperable and have not been registered so far will get registered now. Within registered hierarchy,
