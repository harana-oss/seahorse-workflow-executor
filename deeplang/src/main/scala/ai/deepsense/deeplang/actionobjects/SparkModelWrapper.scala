...
/** Wrapper for creating deeplang Transformers from spark.ml Models. It is parametrized by model and estimator types,
...
abstract class SparkModelWrapper[M <: ml.Model[M], E <: ml.Estimator[M]]
  ...
  /** Model has to be set before _transform() execution. We use a mutable field because model is created by estimator
    * dynamically. Passing it to _transform by parameter is not possible without changing Transformer.transform
    * signature.
    */
  var serializableModel: SerializableSparkModel[M] = _

  /** Parent EstimatorWrapper has to be set before _transformSchema() execution - estimators are responsible for schema
    * inference in models. We use a mutable field because estimator instances that contain fresh parameters are created
    * dynamically. Passing an estimator to _transformSchema by parameter would require a change of
    * Transformer.transformSchema signature.
    */
  ...
  /** Spark model wrappers mustn't have default parameters. All initial parameter values are inherited from parent
    * estimator.
    */
  override protected def setDefault[T](param: Parameter[T], value: T): this.type = this
  ...
  override protected def applyTransform(ctx: ExecutionContext, df: DataFrame): DataFrame =
    DataFrame.fromSparkDataFrame(
      serializableModel.transform(df.sparkDataFrame, sparkParamMap(df.sparkDataFrame.schema))
    )
  ...
  override def replicate(extra: ai.deepsense.deeplang.parameters.ParameterMap): this.type = {
    // If parentEstimator is null then the wrapper was probably
    ...
    val replicatedEstimatorWrapper: SparkEstimatorWrapper[M, E, _] =
      if (parentEstimator == null)
        ...
        parentEstimator
          .replicate(extractParamMap(extra))
  ...
  private def loadParentEstimator(ctx: ExecutionContext, path: String): SparkEstimatorWrapper[M, E, _] =
    ParamsSerialization
      .load(ctx, Transformer.parentEstimatorFilePath(path))
      .asInstanceOf[SparkEstimatorWrapper[M, E, _]]
