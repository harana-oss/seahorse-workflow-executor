...
/** MultiColumnTransformer is a [[ai.deepsense.deeplang.doperables.Transformer]] that can work on either a single column
  * or multiple columns. Also, it can also work in-place (by replacing columns) or not (new columns will be appended to
  * a [[ai.deepsense.deeplang.doperables.dataframe.DataFrame]]). When not working in-place and when working with a
