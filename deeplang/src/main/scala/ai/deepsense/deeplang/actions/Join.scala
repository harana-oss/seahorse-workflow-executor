...
case class Join()
  ...
  val joinType = ChoiceParameter[JoinTypeChoice.Option](name = "join type", description = Some("Type of join operation."))
  ...
  val leftPrefix = new PrefixBasedColumnCreatorParameter(
    name = "left prefix",
  ...
  val rightPrefix = new PrefixBasedColumnCreatorParameter(
    name = "right prefix",
  ...
  val joinColumns =
    ParamsSequence[ColumnPair](name = "join columns", description = Some("Pairs of columns to join upon."))

  def getJoinColumns: Seq[ColumnPair] = $(joinColumns)

  def setJoinColumns(value: Seq[ColumnPair]): this.type = set(joinColumns, value)

  val specificParams: Array[ai.deepsense.deeplang.parameters.Parameter[_]] =
    Array(joinType, leftPrefix, rightPrefix, joinColumns)
  ...
  override protected def execute(ldf: DataFrame, rdf: DataFrame)(context: ExecutionContext): DataFrame = {
    ...
    val renamedJoinColumnsPairs =
      joinColumnsPairs.map { case (leftColumn, rightColumn) =>
    ...
    val noDuplicatesSparkDF =
      sparkDFWithRemovedDuplicatedColumns(joinedDataFrame, renamedJoinColumnsPairs, getColumns(lsdf), getColumns(rsdf))
  ...
  override protected def inferSchema(
    ...
    val prefixedLeftSchema   = StructType(
      leftSchema.map(field => field.copy(name = columnNames.left.originalToRenamed(field.name)))
    )
    val prefixedRightSchema  = StructType(
      rightSchema.map(field => field.copy(name = columnNames.right.originalToRenamed(field.name)))
    )
    val rightJoinColumnNames = getSelectedJoinColumnsNames(rightSchema, _.getRightColumn)

    val columns = prefixedLeftSchema ++
      prefixedRightSchema.filter(col => !rightJoinColumnNames.contains(columnNames.right.renamedToOriginal(col.name)))
  ...
  private def sparkDFWithColumnsRenamed(
    ...
    zipped.foldLeft(initSdf) { (sdf, pair) =>
      val (from, to) = pair
  ...
  private def getSelectedJoinColumnsNames(
      schema: StructType,
      selector: ColumnPair => SingleColumnSelection
  ): Seq[String] =
    getJoinColumns.map(columnPair => DataFrameColumnsGetter.getColumnName(schema, selector(columnPair)))

  private def validateSchemas(leftSchema: StructType, rightSchema: StructType): Unit = {
  ...
  private def prepareJoiningCondition(joinColumns: Seq[(Column, Column)]): Column = {
    ...
    val joinCondition    = joinColumns.foldLeft(initialCondition) { case (acc, (leftColumn, rightColumn)) =>
  ...
  private case class RenamedColumnNames(originalLeftColumns: Seq[String], originalRightColumns: Seq[String]) {
    ...
    case class RightColumnNames(original: Seq[String], prefix: String, leftPrefixed: Seq[String])
...
object Join {
  ...
  case class ColumnPair() extends Params {
    ...
    val leftColumn = SingleColumnSelectorParameter(
      name = "left column",
    ...
    val rightColumn = SingleColumnSelectorParameter(
      name = "right column",
    ...
    val params: Array[ai.deepsense.deeplang.parameters.Parameter[_]] = Array(leftColumn, rightColumn)
