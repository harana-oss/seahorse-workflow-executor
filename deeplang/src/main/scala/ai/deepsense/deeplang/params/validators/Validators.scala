...
case class RangeValidator(
  ...
  /** Counts number of steps that needs to be taken to get to the given value. If number of steps is not an integer then
    * the floor of that number is returned.
    */
  ...
  /** Generates human-readable range constraints to be added as a description. Ex. 1: RangeValidator(0.0, Int.MaxValue,
    * true, false, Some(1))).toHumanReadable("num param"): Range constraints: 0 <= `num param` and `num param` is an
    * integer.
    *
    * Ex. 2: RangeValidator(0.0, 3.0, false, true, Some(0.5))).toHumanReadable("num param"): Range constraints: 0 < `num
    * param` <= 3 and `num param` = k*0.5, where k is an integer.
    */
  ...
  case class Begin() extends Constraint(begin) {
    ...
    override def buildConstraint(oneOfLimits: Boolean, limitRepresentation: String): Option[String] = {
      if (oneOfLimits)
        None
      else
...
object RangeValidator {
  ...
  def positiveIntegers: RangeValidator =
    RangeValidator(begin = 0.0, end = Int.MaxValue, beginIncluded = true, endIncluded = true, step = Some(1.0))

}

case class ArrayLengthValidator(min: Int = 1, max: Int = Int.MaxValue) extends Validator[Array[Double]] {
...
case class RegexValidator(override val regex: Regex) extends RegexValidatorLike
...
/** Validator which accepts a string that can be or be part of proper column name. Column name cannot be empty and must
  * be without backticks. Column with a backtick character fails in some operations (e.g. Convert Type).
  */
