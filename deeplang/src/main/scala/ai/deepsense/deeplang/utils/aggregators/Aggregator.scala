...
trait Aggregator[U, T] {
  ...
  /** Merges elements of type T across one partition in element of type U. Argument `acc` might be mutated and returned
    * for better performance.
    */
  def mergeValue(acc: U, elem: T): U

  /** Merges each partitions `seq` results into one U. Argument `left` might be mutated and returned for better
    * performance.
    */
