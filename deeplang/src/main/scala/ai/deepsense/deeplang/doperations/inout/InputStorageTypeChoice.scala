...
import ai.deepsense.deeplang.params.StorageType
...
object InputStorageTypeChoice {
  ...
  class File extends InputStorageTypeChoice {
    ...
    val sourceFile = LoadFromLibraryParam(name = "source", description = Some("Path to the DataFrame file."))
    ...
    val fileFormat =
      ChoiceParam[InputFileFormatChoice](name = "format", description = Some("Format of the input file."))
