...
/** MultiColumnTransformer is a [[io.deepsense.deeplang.doperables.Transformer]] that can work on either a single column
  * or multiple columns. Also, it can also work in-place (by replacing columns) or not (new columns will be appended to
  * a [[io.deepsense.deeplang.doperables.dataframe.DataFrame]]). When not working in-place and when working with a
...
abstract class MultiColumnTransformer extends Transformer with HasSpecificParams {
  ...
  /** Transforms 'inputColumn' and stores the results in 'outputColumn'. This method should throw an exception when the
    ...
    * transform() Spark
    * [[http://spark.apache.org/docs/latest/api/scala/index.html#org.apache.spark.ml.UnaryTransformer UnaryTransformer]]'s
    * one. If the contract in UnaryTransformer change then this method will have to change, too. Eg. when
    * UnaryTransformer cease to throw on duplicated columns, the contract of this method should be changed accordingly.
    */
