...
case class CustomTransformer(innerWorkflow: InnerWorkflow, publicParamsWithValues: Seq[ParamWithValues[_]])
  ...
  override private[deeplang] def _transformSchema(schema: StructType, inferCtx: InferContext): Option[StructType] = {
    ...
    graphKnowledge
      ...
      .asInstanceOf[DKnowledge[DataFrame]]
      .single
      .schema
  }
  ...
  override protected def loadTransformer(ctx: ExecutionContext, path: String): CustomTransformer.this.type = {
    ...
    CustomTransformerFactory
      .createCustomTransformer(ctx.innerWorkflowExecutor, innerWorkflowJson.asJsObject)
...
case class ParamWithValues[T](param: Param[_], defaultValue: Option[T] = None, setValue: Option[T] = None)
