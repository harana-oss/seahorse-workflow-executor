...
object NumericToVectorUtils {
  ...
  /** Returns schema with modified datatype of specified column (by name). Does not have side-effects. */
  ...
  /** Returns schema with modified datatype of specified column (by index). Does not have side-effects. */
  ...
  /** Converts Schema by changing `inputColumn` type to double. `convertOutputVectorToDouble` allows converting
    * `outputColumn` to double
    */
  def revertSchema(
      schema: StructType,
      inputColumn: String,
      outputColumn: String,
      convertOutputVectorToDouble: Boolean
  ): StructType = {
  ...
  /** Converts DataFrame by changing `inputColumn` type to double. `convertOutputVectorToDouble` allows converting
    * `outputColumn` to double
    */
  def revertDataFrame(
    ...
    val extractFirstValueFromVector = (columnIdx: Int) =>
      (r: Row) => {
        ...
        if (vector != null)
          Row.fromSeq(r.toSeq.updated(columnIdx, vector.asInstanceOf[io.deepsense.sparkutils.Linalg.Vector].apply(0)))
        else
