...
trait Params extends Serializable with HasInferenceResult with DefaultJsonProtocol with Logging {
  ...
  def paramPairsFromJson(jsValue: JsValue): Seq[ParamPair[_]] = jsValue match {
    case JsObject(map) =>
      val pairs = for ((label, value) <- map) yield (paramsByName.get(label), value) match {
        ...
        case (Some(parameter), _) =>
          if (isMultiValueParam(parameter, value))
            ...
            Some(ParamPair(parameter.asInstanceOf[Param[Any]], parameter.valueFromJson(value)))
        case (None, _) =>
  ...
  private def getMultiValueParam(value: JsValue, parameter: Param[_]): Option[ParamPair[_]] =
    parameter match {
      case _: IntParamWrapper[_] | _: LongParamWrapper[_] | _: FloatParamWrapper[_] | _: DoubleParamWrapper[_] |
          _: NumericParam =>
  ...
  private def createMultiValueParam[T](value: JsValue, parameter: Param[_])(implicit
      format: JsonFormat[T]
  ): Option[ParamPair[T]] = {
  ...
  final protected def set[T](param: Param[T], value: T): this.type =
    set(param -> value)
  ...
  final protected[deeplang] def set(paramMap: ParamMap): this.type = {
    set(paramMap.toSeq: _*)
    this
  }

  final protected def clear(param: Param[_]): this.type = {
  ...
  final protected def $[T](param: Param[T]): T = getOrDefault(param)
  ...
  protected def setDefault(paramPairs: ParamPair[_]*): this.type = {
    paramPairs.foreach(p => setDefault(p.param.asInstanceOf[Param[Any]], p.value))
    this
  }
