...
/** In CSV there are no type hints/formats. Everything is plain text between separators.
  ...
  *
  * Logic here looks for the most strict common subtype of all values per each column and converts schema to have more
  * rich types than strings.
  */
object CsvSchemaInferencerAfterReading {

  def postprocess(
      csvChoice: InputFileFormatChoice.Csv
  )(sparkDataFrame: SparkDataFrame)(implicit context: ExecutionContext): SparkDataFrame = {
    ...
    val convertedData = rawStringData.map(row =>
      Row.fromSeq {
        row.toSeq.zipWithIndex.zip(inferredTypes).map { case ((cell: String, index), inferredType) =>
          convertCell(cell, inferredType)
        }
      }
    )
  ...
  private def inferTypes(csvChoice: InputFileFormatChoice.Csv)(data: RDD[Row]): Seq[DataType] = {
    ...
    val linesInferences =
      data.map(_.toSeq.map { case cell: String =>
        cellTypeInference(cell, shouldConvertToBoolean)
      })
  ...
  private def buildSchema(columnNames: Seq[String], inferredTypes: Seq[DataType]): StructType =
    StructType(columnNames.zip(inferredTypes).map { case (columnName, inferredType) =>
  ...
  private def renameUnnamed(maybeEmptyNames: Seq[String]): Seq[String] = {
    val (columnNames, _) = maybeEmptyNames.foldLeft((Seq[String](), maybeEmptyNames.toSet)) {
      case ((columnsNames, usedNames), inputColumnName) =>
        val columnName =
          if (StringUtils.isBlank(inputColumnName))
