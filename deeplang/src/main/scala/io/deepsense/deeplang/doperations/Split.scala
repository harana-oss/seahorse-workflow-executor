...
case class Split() extends DOperation1To2[DataFrame, DataFrame, DataFrame] with Params with OperationDocumentation {
  ...
  private def executeConditionalSplit(
    ...
    val results =
      Future.sequence(Seq(Future(runExpression(leftExpression)), Future(runExpression(rightExpression)))).map {
        case Seq(leftDataFrame, rightDataFrame) =>
  ...
  override protected def inferKnowledge(knowledge: DKnowledge[DataFrame])(
