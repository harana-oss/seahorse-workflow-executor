...
class SingleColumnSelectorParamSpec extends AbstractParamSpec[SingleColumnSelection, SingleColumnSelectorParam] {
  ...
  override def paramFixture: (SingleColumnSelectorParam, JsValue) = {
    ...
    val param =
      SingleColumnSelectorParam(name = "Single column selector name", description = Some(description), portIndex = 0)
