  ...
  "Transformer" should {
    ...
    "infer schema" when {
      "it's implemented" when {
        ...
        val expectedOutputDKnowledge = Knowledge(DataFrame.forInference(outputSchema))
        "input DKnowledge contains exactly one type" in {
          val inputDKnowledge = Knowledge(DataFrame.forInference(inputSchema))
    ...
    "not infer schema" when {
      "it's not implemented" in {
        ...
        val inputDKnowledge          = Knowledge(DataFrame.forInference(inputSchema))
        val expectedOutputDKnowledge = Knowledge(DataFrame.forInference(None))
        val output                   = t.transform.infer(inferCtx)(())(inputDKnowledge)
        output shouldBe (expectedOutputDKnowledge, emptyWarnings)
      }
    }
