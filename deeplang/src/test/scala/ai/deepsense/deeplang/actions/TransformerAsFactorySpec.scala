...
class TransformerAsFactorySpec extends UnitSpec {
  ...
  "TransformerAsFactory" should {
    ...
    "have defaults same as in Transformer" in {
      ...
      op.extractParamMap() shouldBe ParameterMap(transformerParam, ReportTypeDefault(op.reportType))
    }
    "produce transformer with properly set params" in {
      ...
      result(0).asInstanceOf[MockTransformer].extractParamMap() shouldBe
        ParameterMap(op.transformer.paramA -> 2, ReportTypeDefault(op.reportType))
    }
    "infer knowledge" in {
      val op = operation
      op.set(op.transformer.paramA -> 2)
      ...
      val (result, warnings) =
        op.inferKnowledgeUntyped(Vector(Knowledge()))(mock[InferContext])
