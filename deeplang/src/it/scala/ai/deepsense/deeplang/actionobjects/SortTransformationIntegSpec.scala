...
import ai.deepsense.deeplang.actionobjects.spark.wrappers.transformers.TransformerSerialization.TransformerSerializationOps
  ...
  trait SampleData {
    ...
    def generateOrderingAndSortColumnParams(
      ...
      (
        ...
        columnIndices.map { case OrderingSpec(i, desc, _) => SortColumnParam(i, desc) }
      )
    }
