    ...
    "throw exceptions" when {
      ...
      "prediction column has invalid type and there is no rawPrediction column" in {
        ...
        intercept[ColumnDoesNotExistException] {
          eval.setMetricName(new AreaUnderROC())._infer(Knowledge(invalidDataFrame))
        }
        intercept[ColumnDoesNotExistException] {
          eval.setMetricName(new AreaUnderPR())._infer(Knowledge(invalidDataFrame))
        }
        intercept[WrongColumnTypeException] {
          eval.setMetricName(new Precision())._infer(Knowledge(invalidDataFrame))
        }
        intercept[WrongColumnTypeException] {
          eval.setMetricName(new Recall())._infer(Knowledge(invalidDataFrame))
        }
        intercept[WrongColumnTypeException] {
          eval.setMetricName(new F1Score())._infer(Knowledge(invalidDataFrame))
        }
        ()
      }
