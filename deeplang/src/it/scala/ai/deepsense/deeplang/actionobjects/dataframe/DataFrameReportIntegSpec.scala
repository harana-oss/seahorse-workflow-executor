  ...
  "DataFrame" should {
    "generate report with data sample table" when {
      ...
      "DataFrame has missing values" in {
        ...
        val rdd: RDD[Row]       =
          sparkContext.parallelize(List(Row(null, new Timestamp(now.getMillis)), Row(exampleString, null)))
        val schema: StructType  =
          StructType(List(StructField(nameColumnName, StringType), StructField(birthDateColumnName, TimestampType)))
        val dataFrame           =
    ...
    "generate correct report" when {
      "DataFrame is empty" in {
        ...
        val emptyDataFrame =
          executionContext.dataFrameBuilder.buildDataFrame(schema, sparkContext.parallelize(Seq.empty[Row]))
      ...
      "DataFrame consists of null values only" in {
        ...
        val emptyDataFrame = executionContext.dataFrameBuilder.buildDataFrame(
          ...
          sparkContext.parallelize(
            Seq(Row(null, null, null, null, null), Row(null, null, null, null, null), Row(null, null, null, null, null))
          )
        )
