...
object ExampleHtmlFormatter {
  ...
  def exampleHtml(operation: Action, inputDataFrames: Seq[DataFrame], outputDataFrames: Seq[DataFrame]): String = {
  ...
  private def paramsHtml(operation: Action): String =
...
object ParametersHtmlFormatter {
  ...
  def toHtml(dOperation: Action): String = {
    if (dOperation.paramValuesToJson.asJsObject.fields.isEmpty)
      ...
      val orderedValues = paramsOrder.flatMap { case paramName =>
        paramValues.get(paramName).map(v => paramName -> v)
      }

      val paramsHtml = orderedValues.map { case (paramName, paramValue) =>
  ...
  private def extractParamsOrder(json: JsArray): Seq[String] = {
    json.elements.flatMap { case param =>
      ...
      val subParams   =
        if (
          paramObject.fields.contains(typeFieldName) &&
          paramObject.fields(typeFieldName).asInstanceOf[JsString].value == "choice"
        ) {
          val choices = paramObject.fields(valuesFieldName).asInstanceOf[JsArray].elements.map(_.asJsObject).toSeq
  ...
  private def extractParamValues(jsObject: JsObject): Seq[(String, String)] = {
    jsObject.fields.toSeq.flatMap {
      case (name, value: JsArray) if name == userDefinedMissingValues && isUserDefinedMissingValue(value) =>
  ...
  private def handleUserDefinedMissingValues(name: String, missingValues: JsArray): (String, String) = {
  ...
  private def handleColumnProjection(name: String, projectionsJs: JsArray): (String, String) = {
    val pairs = projectionsJs.elements.map { v =>
      ...
      val renameDesc     =
        if (
          v.asJsObject.fields.contains(Projector.RenameColumnParameterName) &&
          v.asJsObject.fields(Projector.RenameColumnParameterName).asJsObject.fields.contains("Yes")
        ) {
          " (renamed to <code>" +
            v.asJsObject
              ...
              .get
              .asJsObject
              .fields(Projector.ColumnNameParameterName)
              .asInstanceOf[JsString]
              .value +
            "</code>)"
        } else
  ...
  private def handleSortColumnParams(name: String, sortColumnParams: JsArray): (String, String) = {
    val sortColumns = sortColumnParams.elements.map { v =>
      val colName = extractColumnName(v, SortColumnParam.columnNameParamName)
      val ascDesc =
        if (v.asJsObject.fields(SortColumnParam.descendingFlagParamName).asInstanceOf[JsBoolean].value)
          "DESC"
        else
          "ASC"
      s"$colName $ascDesc"
  ...
  private def handleMultipleColumnSelection(name: String, selectionsJs: JsObject): (String, String) = {
