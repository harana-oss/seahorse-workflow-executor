...
class DatetimeDecomposerIntegSpec extends DeeplangIntegTestSupport with TransformerSerialization {
  ...
  private def shouldTransformSchema(schema: StructType, prefix: String): Unit = {
    val operation: DatetimeDecomposer = operationWithParamsSet(prefix)
  ...
  private def decomposeDatetime(decomposeDatetime: Transformer, dataFrame: DataFrame): DataFrame =
