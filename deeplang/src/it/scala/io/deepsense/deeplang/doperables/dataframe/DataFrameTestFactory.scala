...
trait DataFrameTestFactory {
  ...
  def oneValueDataFrame(dataFrameBuilder: DataFrameBuilder, sparkContext: SparkContext): DataFrame =
  ...
  val allTypesSchema: StructType = StructType(
    Seq(
      ...
      StructField(vectorColumnName, new io.deepsense.sparkutils.Linalg.VectorUDT)
    )
  )
