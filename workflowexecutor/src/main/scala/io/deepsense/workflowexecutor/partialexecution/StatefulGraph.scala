...
case class StatefulGraph(
  ...
  /** Tells the graph that the execution has failed. Eg. there was an issue other than node exception that makes
  ...
  def updateStates(changedGraph: StatefulGraph): StatefulGraph = {
    ...
    copy(states = updatedStates, executionFailure = changedGraph.executionFailure)
  }

  def subgraph(nodes: Set[Node.Id]): StatefulGraph = {
    val directedsubgraph = directedGraph.subgraph(nodes)
    val substatuses = states.filter { case (id, _) =>
      directedsubgraph.nodes.map(_.id).contains(id)
    }
    ...
    if (predecessorsReady(id)) {
      val entities = directedGraph.predecessors(id).flatten.map { case Endpoint(predecessorId, portIndex) =>
        states(predecessorId) match {
          case NodeStateWithResults(NodeState(nodestate.Completed(_, _, results), _), dOperables, _) =>
  ...
  private def changeState(id: Node.Id)(f: (NodeStateWithResults) => NodeStateWithResults): StatefulGraph = {
    val updatedStates = {
      ...
      val successorsOfFailedAborted =
        if (newNodeState.isFailed)
  ...
  private def abortUnfinished(unfinished: Map[Id, NodeStateWithResults]): Map[Id, NodeStateWithResults] =
