...
import io.deepsense.models.workflows.NodeStateWithResults
...
case class IdleExecution(override val graph: StatefulGraph, selectedNodes: Set[Node.Id] = Set.empty) extends Execution {
  ...
  private def findStates(newStructure: DeeplangGraph, substructure: DeeplangGraph, nodes: Set[Node.Id]): NodeStates = {
    ...
    if (newStructure.containsCycle)
      ...
      val predecessorsChangedNodes = newStructure.nodes
        .map(_.id)
        .diff(newNodes)
        .filter(id => newStructure.predecessors(id) != graph.predecessors(id))
  ...
  private def draftNodes(nodesNeedingDrafting: Set[Node.Id])(graph: StatefulGraph): StatefulGraph =
    nodesNeedingDrafting.foldLeft(graph) { case (g, id) =>
      g.draft(id)
    }

  private def clearNodesKnowledge(nodesToClear: Set[Node.Id])(graph: StatefulGraph): StatefulGraph =
    nodesToClear.foldLeft(graph) { case (g, id) =>
      g.clearKnowledge(id)
    }

}

abstract class StartedExecution(fullGraph: StatefulGraph, runningPart: StatefulGraph, selectedNodes: Set[Node.Id])
    extends Execution {
  ...
  final def withRunningPartUpdated(update: (StatefulGraph) => StatefulGraph): Execution = {
...
case class RunningExecution(fullGraph: StatefulGraph, runningPart: StatefulGraph, selectedNodes: Set[Node.Id])
  ...
  override protected def copyGraphs(updatedRunningPart: StatefulGraph, updatedGraph: StatefulGraph): Execution =
    RunningExecution(updatedGraph, updatedRunningPart, selectedNodes)

}

case class AbortedExecution(val fullGraph: StatefulGraph, runningPart: StatefulGraph, selectedNodes: Set[Node.Id])
    extends StartedExecution(fullGraph, runningPart, selectedNodes) {
  ...
  override protected def copyGraphs(updatedRunningPart: StatefulGraph, updatedFullGraph: StatefulGraph): Execution =
