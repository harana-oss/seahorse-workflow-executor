...
import io.deepsense.workflowexecutor.executor.SessionExecutor
...
object WorkflowExecutorApp extends Logging {
  ...
  private val parser: OptionParser[ExecutionParams] = new scopt.OptionParser[ExecutionParams](BuildInfo.name) {
    ...
    // Hidden option:
    opt[String]("workflow-id").hidden().valueName("JOB").action((x, c) => c.copy(workflowId = Some(x))).text("job id")
    ...
    opt[String]("mail-server-host")
      .hidden()
      .valueName("HOST")
      .action((x, c) => c.copy(mailParams = c.mailParams.copy(mailServerHost = Some(x))))

    opt[Int]("mail-server-port")
      .hidden()
      .valueName("PORT")
      .action((x, c) => c.copy(mailParams = c.mailParams.copy(mailServerPort = Some(x))))

    opt[String]("mail-server-user")
      .hidden()
      .valueName("USER")
      .action((x, c) => c.copy(mailParams = c.mailParams.copy(mailServerUser = Some(x))))

    opt[String]("mail-server-password")
      .hidden()
      .valueName("PASS")
      .action((x, c) => c.copy(mailParams = c.mailParams.copy(mailServerPassword = Some(x))))

    opt[String]("mail-server-sender")
      .hidden()
      .valueName("EMAIL")
      .action((x, c) => c.copy(mailParams = c.mailParams.copy(mailServerSender = Some(x))))

    opt[String]("notebook-server-address")
      .hidden()
      .valueName("URL")
      .action((x, c) => c.copy(notebookServerAddress = Some(new URL(x))))

    opt[String]("datasource-server-address")
      .hidden()
      .valueName("URL")
      .action((x, c) => c.copy(datasourceServerAddress = Some(new URL(x))))
