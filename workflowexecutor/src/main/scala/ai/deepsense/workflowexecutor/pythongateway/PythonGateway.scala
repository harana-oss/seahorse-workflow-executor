...
case class PythonGateway(
  ...
  private def createGatewayServer(entryPoint: CustomCodeEntryPoint, listener: GatewayEventListener): GatewayServer = {

    val callbackClient =
      new LazyCallbackClient(() => entryPoint.getPythonPort(gatewayConfig.pyExecutorSetupTimeout), hostAddress)
    ...
    val gateway =
      GatewayServerFactory.create(entryPoint, port, connectTimeout, readTimeout, callbackClient, hostAddress)
