...
import ai.deepsense.workflowexecutor.exception.WorkflowExecutionException
...
case class WorkflowExecutor(
  ...
  def execute(sparkContext: SparkContext): Try[ExecutionReport] = Try {
    ...
    val pythonExecutionCaretaker = new PythonExecutionCaretaker(customCodeExecutorsPath, pythonPathGenerator,
      pythonBinary, sparkContext, sparkSQLSession, dataFrameStorage, customCodeEntryPoint, hostAddress)
