...
import ai.deepsense.commons.mail.EmailSenderAuthorizationConfig
import ai.deepsense.commons.mail.EmailSenderConfig
...
import ai.deepsense.workflowexecutor.communication.mq.serialization.json.ProtocolJsonDeserializer
import ai.deepsense.workflowexecutor.communication.mq.serialization.json.ProtocolJsonSerializer
...
import ai.deepsense.workflowexecutor.WorkflowManagerClientActor
...
case class SessionExecutor(
  ...
  def execute(): Unit = {
    ...
    val workflowsSubscriberActor: ActorRef =
      createWorkflowsSubscriberActor(sparkContext, sparkSQLSession, dOperableCatalog, dataFrameStorage,
        customCodeExecutionProvider, system, workflowManagerClientActor, communicationFactory)
