...
import ai.deepsense.workflowexecutor.communication.message.global.Ready
...
class SessionWorkflowExecutorActor(
  ...
  override def receive: Receive = { case Init() =>
    initiate()
  }
  ...
  private def scheduleHeartbeats(): Unit = {
    ...
    scheduledHeartbeat = Some(
      context.system.scheduler.schedule(Duration.Zero, heartbeatInterval, heartbeatPublisher, heartbeat)
    )
  }
...
object SessionWorkflowExecutorActor {
  ...
  def props(
    ...
    Props(
      new SessionWorkflowExecutorActor(ec, new GraphNodeExecutorFactoryImpl, workflowManagerClientActor, publisher,
        heartbeatPublisher, notebookPublisher, wmTimeout, sessionId, heartbeatInterval)
    )
