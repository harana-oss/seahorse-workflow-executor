...
import com.newmotion.akka.rabbitmq.ChannelActor.Connected
import com.newmotion.akka.rabbitmq.ChannelActor.Disconnected
...
/** A class used as a way to return data from the setupChannel callback
  *
  * @param data
  *   whatever the setupChannel returned
  * @param channelActor
  *   the channel that was just set up
  * @tparam T
  *   setupChannel return type
  */
...
case class MQCommunicationFactory(
  ...
  /** @return
    *   future containing queue name and channel actor so we can later unsubscribe and close the channel
    */
...
object MQCommunicationFactory {
  ...
  /** NotifyingChannelActor extends ChannelActor behaviour by completing a promise after connecting to a channel. In the
    * newmotion's library for RMQ, it's impossible to synchronously connect to a channel. NotifyingChannelActor resolves
    * this issue by allowing to wait for the promise to complete.
    *
  ...
  object NotifyingChannelActor {
    ...
    def props[T](channelConnected: Promise[ChannelSetupResult[T]], setupChannel: (Channel, ActorRef) => T): Props =
