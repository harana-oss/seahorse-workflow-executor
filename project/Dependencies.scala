...
object Version {
  ...
  val spark = sys.props.getOrElse("SPARK_VERSION", "2.4.8") // TEMPORAL

  val (scala, java, hadoop, akka) = spark match {
    case "2.4.8" => ("2.11.12", "1.8", "2.7.1", "2.5.32")
  }
...
object Library {
  ...
  val wireMock =
    ("com.github.tomakehurst" % "wiremock" % Version.wireMock).exclude("com.google.guava", "guava") excludeJackson
...
object Dependencies {
  ...
  val resolvers = Seq(
    ...
    "sonatype.org".at("https://oss.sonatype.org/content/repositories/releases"), "spray.io".at("https://repo.spray.io"),
  ...
  val commons = usedSpark.onlyInTests ++ Seq(
    akkaActor, sprayClient, apacheCommonsLang3, config, javaMail, log4JExtras, nscalaTime, scalate, slf4j, slf4jLog4j,
    sprayCan, sprayHttpx, sprayJson
  ) ++ Library.testing

  val deeplang = usedSpark.onlyInTests ++ Hadoop.onlyInTests ++ GoogleServicesApi.components ++ Seq(akkaActor,
    sprayClient, apacheCommonsLang3, amazonS3, nscalaTime, scalaReflect, apacheCommonsCsv, reflections) ++
    Library.testing

  val docgen = usedSpark.components

  val graph = Seq(nscalaTime) ++ Library.testing
  ...
  val workflowexecutor = usedSpark.onlyInTests ++ Seq(
    akkaActor, jsonLenses, scopt, sprayClient, rabbitmq
  ) ++ Seq(akkaTestkit, mockitoCore, scalatestCheck, scalatestMockito, scalatest, wireMock).map(_ % s"$Test,it")
